...
namespace GOAP
{
    ...
    public class G_BoolState : G_State
    {
        ...
        public override object ConvertSerializedStringToValue(string serializedString)
        {
            ...
            if(type == 'b')
            {
                return bool.Parse(serializedString.Substring(1));
            }
            else
            {
                return null;
        ...
        /// <summary>
        /// Test the given state's value againts the expected value using the given comparion and return true
        ...
        public override bool TestStateConditionMatch(G_Condition precondition, G_Condition effect)
        {
            ...
            // Avoiding errors due to invalid comparions
            if (!preCompareValid || !effectCompareValid)
            {
                Debug.LogWarning($"Invalid comparions found when comparing one or more conditions for bool state");
            }
            ...
            GUIContent label)
        {
            ...
            if (toggleValue != (bool)expectedValue.managedReferenceValue)
            {
                ...
                property.FindPropertyRelative("serializedExpectedValue").stringValue = "b" + toggleValue.ToString();
            }
