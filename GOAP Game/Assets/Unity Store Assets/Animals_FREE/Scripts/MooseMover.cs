...
public class MooseMover : MonoBehaviour
{
    [Header("Waypoints (local space relative to origin)")]
    [SerializeField] private Vector3[] points;
    [SerializeField] private Transform masterDeer;
    private Vector3 worldOffset = new Vector3(-7.41426992f, 0f, 4.73939991f);
    private Vector3 masterOrigin;
    private Vector3 deerOffset;

    [Header("Origin Reference (optional)")]
    ...
    {
        if (masterDeer != null)
            masterOrigin = masterDeer.position;

        deerOffset = transform.position - masterDeer.position;
    ...
    {
        return worldOffset + point + deerOffset;
    }
